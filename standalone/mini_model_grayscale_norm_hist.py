...
def img_grayscale_norm_hist(img, target_hist, M, S):
    arr = lum_norm(img, M, S, target_hist)
    mask = arr[:, :, 3] > 0
    pixels = arr[mask].astype(np.float32) / 255.0
    pixels = pixels[:, :3]
    h, _ = np.histogram(pixels[:, 1], bins=255, range=(0, 1))
    h = h / h.sum()
...
class PathsAndLabels(Dataset):
    def __init__(self, paths: List[str], labels: np.ndarray, root: Optional[str] = None):
        ...
        for i in tqdm(range(len(self.paths))):
            ...
            self.images[i] = img_grayscale_norm_hist(Image.open(full_p).convert("RGBA"), target_hist, M, S)
